...
namespace Demo
{
    ...
    public static partial class FixtureWidePrimitives
    {
        ...
        /// <summary>Reads the records of <paramref name="source"/> - one <c>root</c> after another until the memory ends - on the enumeration step that reaches each; each record is 12 bytes. Trailing bytes shorter than one record fail on the step that meets them; a failure names the record by its index before the path (<c>[3].root</c>). Each record is its own region: the read limits apply per record and a stored absolute pointer address counts from the record's first byte.</summary>
        ...
        public sealed partial class Root : global::CStructSharp.ICStructGenerated<Root>
        {
            ...
            /// <inheritdoc/>
            static bool global::CStructSharp.ICStructGenerated<Root>.TryParse(global::System.ReadOnlySpan<byte> source, [global::System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)] out Root value, out global::CStructSharp.Diagnostics.CStructException? failure, global::CStructSharp.ReadOptions? options) => FixtureWidePrimitives.TryParse(source, out value, out failure, options);
        ...
        public readonly ref struct RootView
        {
            ...
            /// <summary>Creates a view over <paramref name="source"/>, whose first byte is the value's first byte; the source must hold the value's 12 bytes.</summary>
            ...
            public RootView(global::System.ReadOnlySpan<byte> source, global::CStructSharp.ReadOptions? options = null)
            {
                ...
                {
                    var cursor = new global::CStructSharp.Generated.ReadCursor(source, options, "root");
                    global::CStructSharp.Diagnostics.CStructException failure = cursor.Fail(global::CStructSharp.Generated.ReadCursor.ShortReadText(12, source.Length), null, null);
            ...
            /// <summary>Gets the value's bytes (its 12 bytes).</summary>
            public global::System.ReadOnlySpan<byte> Bytes => this.source.Slice(0, 12);

            /// <summary>The stored address of <c>void *p</c> at offset 8 (the target is read by <see cref="ToObject"/>).</summary>
            public long PAddress => (long)global::CStructSharp.Generated.Codec.ReadUnsigned(this.source.Slice(8, PointerSize), LittleEndian);
            ...
            /// <summary>Views over the records of <paramref name="source"/> - one <c>root</c> of 12 bytes after another until the source ends - for <c>foreach</c>, allocating nothing. Trailing bytes shorter than one record fail on the step that meets them; each record is its own region (a stored absolute pointer address counts from the record's first byte).</summary>
            /// <param name="source">The bytes of the records, with nothing else after them.</param>
        ...
        /// <summary>Walks consecutive <c>root</c> records of 12 bytes; trailing bytes shorter than one record fail on the step that meets them, with the record's index in the path.</summary>
        public ref struct RootViewEnumerator
        {
            ...
            {
                int next = this.index < 0 ? 0 : this.offset + 12;
                if (next >= this.source.Length)
                {
                    return false;
                }
                if (this.source.Length - next < 12)
                {
                    throw global::CStructSharp.Generated.RecordSequence.Partial(this.source.Length - next, 12, "root", this.index + 1, next);
                }
                this.offset = next;
                this.index++;
                return true;
