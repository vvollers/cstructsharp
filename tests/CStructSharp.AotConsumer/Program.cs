...
// A typed read into a POCO with a nested class, a nested array, and a scalar array. The nested class carries the
// documented preservation attribute; the root type is preserved by ReadValue<T>'s own annotation.
...
Check(record.Tag == 7, "poco tag");
Check(record.Origin.X == -2 && record.Origin.Y == 5, "poco nested class");
Check(record.Corners.Length == 2 && record.Corners[1].X == 3 && record.Corners[1].Y == 4, "poco nested array");
Check(record.Flags.SequenceEqual(new byte[] { 1, 2, 3 }), "poco scalar array");
...
Check(parsed.Get<Records.Point>("origin").X == -2, "path get poco");
...
// Writes from a POCO, a dictionary, and a parsed value; an update in place.
byte[] written = records.Serialize("record", record);
Check(written.AsSpan().SequenceEqual(recordBytes), "poco serialize round trip");
...
// A collection-interface member needs a run-time List<T>, which Native AOT cannot instantiate; the error says so.
try
{
    records.ReadValue<Records.InterfaceRecord>(recordBytes, "record");
    Check(false, "IList<T> member mapped without dynamic code");
}
catch (CStructReadException error)
{
    Check(error.Message.Contains("List<Point>", StringComparison.Ordinal), "IList<T> guidance: " + error.Message);
}
