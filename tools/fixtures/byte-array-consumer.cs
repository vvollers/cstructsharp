...
byte[] bytes = { 2, 0 };
if (layout.Parse(bytes).Get<ushort>("value") != 2 ||
    !Equals(layout.ReadValue(bytes, "root.value"), (ushort)2) ||
    layout.ReadValue<ushort>(bytes, "root.value") != 2 ||
    !layout.TryReadValue(bytes, "root.value", out ushort value) || value != 2)
...
if (layout.ReadValue<ushort>(bytes.AsSpan(), "root.value") != 2 ||
    ...
    layout.TryReadValue(Array.Empty<byte>(), "root.value", out value))
...
Action[] nullCalls = {
    ...
    () => layout.TryReadValue((byte[])null!, null, out ushort _),
};
