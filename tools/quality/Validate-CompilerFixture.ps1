...
param(
    [string[]]$EvidencePath = @(
        (Join-Path $PSScriptRoot '../../contracts/quality/compiler-fixtures/baselines')
    ),
...
Assert-Condition ($contract.runner -eq 'tools/quality/Invoke-CompilerDifferentialFixture.ps1') `
    'The feature-operation matrix names an unexpected compiler fixture runner.'
Assert-Condition ($contract.validator -eq 'tools/quality/Validate-CompilerFixture.ps1') `
