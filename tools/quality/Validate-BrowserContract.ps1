...
$baselinePath = Join-Path $RepositoryRoot 'contracts/api/browser-rc1/contract.json'
$contractPath = Join-Path $RepositoryRoot 'apps/workshop/src/wasm/cstruct-contract.ts'
$boundaryPath = Join-Path $RepositoryRoot 'src/CStructSharp.Wasm/CStructInteropBoundary.cs'
$exportsPath = Join-Path $RepositoryRoot 'src/CStructSharp.Wasm/CStructExports.cs'
$bootstrapPath = Join-Path $RepositoryRoot 'src/CStructSharp.Wasm/bootstrap.js'
$packagePath = Join-Path $RepositoryRoot 'apps/workshop/package.json'
$dtoPaths = @(
    ...
    'src/CStructSharp.Wasm/CStructJsonContext.cs') | ForEach-Object { Join-Path $RepositoryRoot $_ }
