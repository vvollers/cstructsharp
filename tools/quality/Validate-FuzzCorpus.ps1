[CmdletBinding()]
param(
    [string]$CorpusPath = (Join-Path $PSScriptRoot '../../tests/CStructSharp.Fuzz/corpus/fuzz-corpus.json'),
...
Assert-Condition ($contract.corpus -eq 'tests/CStructSharp.Fuzz/corpus/fuzz-corpus.json') `
    'The feature-operation matrix names an unexpected fuzz corpus.'
Assert-Condition ($contract.project -eq 'tests/CStructSharp.Fuzz/CStructSharp.Fuzz.csproj') `
    'The feature-operation matrix names an unexpected fuzz project.'
Assert-Condition ($contract.validator -eq 'tools/quality/Validate-FuzzCorpus.ps1') `
    'The feature-operation matrix names an unexpected fuzz validator.'
Assert-Condition ($contract.guide -eq 'docs/project/testing.md') `
