...
Assert-Condition ([string]$config.project -eq 'src/CStructSharp/CStructSharp.csproj') `
...
Assert-Condition (@($config.'test-projects').Count -eq 1 -and
                  [string]$config.'test-projects'[0] -eq 'tests/CStructSharpTests/CStructSharpTests.csproj') `
