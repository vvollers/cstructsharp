[CmdletBinding()]
param(
    [string]$ContractPath = (Join-Path $PSScriptRoot '../../contracts/language/portable-v1.json'),
    [string]$ReferencePath = (Join-Path $PSScriptRoot '../../docs/language/index.md'),
    [string]$MatrixPath = (Join-Path $PSScriptRoot '../../contracts/quality/feature-operation-matrix.json')
)
...
Assert-Condition ($matrix.canonicalReference.contract -eq 'contracts/language/portable-v1.json') `
...
Assert-Condition ($matrix.canonicalReference.reference -eq 'docs/language/index.md') `
    'The feature-operation matrix has an unexpected human reference path.'
Assert-Condition ($matrix.canonicalReference.validator -eq 'tools/documentation/Validate-CanonicalReference.ps1') `
