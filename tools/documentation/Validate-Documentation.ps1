...
$TestProjectPath = Join-Path $RepositoryRoot 'tests/CStructSharpTests/CStructSharpTests.csproj'
...
function Get-IgnoredDocumentationDependencies {
    ...
    $ignoredDocsPattern = '(?i)(?:^|[\s"''`()=:])(?:agentdocs|\.local-docs)[\\/]'
...
Assert-Condition ($generatedRecipes.Count -eq 46) 'Expected all 46 recipe exports.'
...
Assert-Condition ($exampleReferences.Count -eq 1 -and
                  $exampleReferences[0].Include -eq '..\..\src\CStructSharp\CStructSharp.csproj') `
...
{
    $docRelative = [IO.Path]::GetRelativePath($DocumentationRoot, $sourceFile.FullName).Replace('\', '/')
    if ($docRelative -in $generatedRecipes) { continue }
...
$expectedContracts = @(Get-Content (Join-Path $contractDirectory 'published-files.json') -Raw | ConvertFrom-Json)
$actualContracts = @($contractSourceFiles | ForEach-Object { [IO.Path]::GetRelativePath($contractDirectory, $_.FullName).Replace('\', '/') } | Where-Object { $_ -ne 'published-files.json' })
Assert-Condition (@(Compare-Object $expectedContracts $actualContracts).Count -eq 0) 'Published contract inventory differs from required inputs.'
foreach ($contractSource in $contractSourceFiles)
{
    $relativeContract = ('contracts/' + [IO.Path]::GetRelativePath($contractDirectory, $contractSource.FullName))
