...
Assert-Condition (
    $apiPage -match 'https://github\.com/vvollers/CStructSharp/blob/[0-9a-f]{40}/src/CStructSharp/CStruct\.cs' -or
    $templateScript.Contains(
        "$($contract.repositoryUrl)/blob/$($contract.defaultBranch)/src/CStructSharp/CStruct.cs",
...
Assert-Condition ($templateScript.Contains(
    "$($contract.repositoryUrl)/edit/$($contract.defaultBranch)/docs/",
...
[xml]$coreProject = Get-Content -LiteralPath (
    Join-Path $RepositoryRoot 'src/CStructSharp/CStructSharp.csproj') -Raw
