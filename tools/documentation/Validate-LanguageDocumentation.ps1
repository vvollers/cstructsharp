...
param(
    ...
    [string]$FixturePath = (Join-Path $PSScriptRoot '../../contracts/language/manual-fixtures-v1.json'),
    [string]$MatrixPath = (Join-Path $PSScriptRoot '../../contracts/quality/feature-operation-matrix.json'),
    [string]$LanguageRoot = (Join-Path $PSScriptRoot '../../docs/language')
)
...
function Assert-ManualReference {
    ...
    Assert-Condition ($parts[0] -match '^docs/language/.+\.md$') `
