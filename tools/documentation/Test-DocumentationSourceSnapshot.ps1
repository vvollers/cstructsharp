...
try
{
    ...
    $deletedFiles = @(& git -C $RepositoryRoot diff HEAD --no-renames --name-only --diff-filter=D)
    ...
    Assert-Condition (-not (Test-Path -LiteralPath (
        Join-Path $SnapshotRoot ([IO.Path]::Combine('agentdocs', 'DOCUMENTATION_PLAN.md'))))) `
    ...
    try
    {
        ...
        & pwsh -NoProfile -File './tools/documentation/Validate-Documentation.ps1'
        ...
        $apiPage = Get-Content -LiteralPath (
            Join-Path $SnapshotRoot 'docs/_site/api/CStructSharp.CStruct.html') -Raw
        Assert-Condition ($apiPage -match
            'https://github\.com/vvollers/CStructSharp/blob/[0-9a-f]{40}/src/CStructSharp/CStruct\.cs') `
        ...
        & pwsh -NoProfile -File './tools/documentation/New-DocumentationPagesArtifact.ps1'
