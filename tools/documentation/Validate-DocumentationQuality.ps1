...
$FixturePath = Join-Path $RepositoryRoot 'contracts/documentation/validator-fixtures.json'
...
))
{
    ...
    $generatedFiles = @(Get-Content (Join-Path $DocumentationRoot 'generated-files.json') -Raw | ConvertFrom-Json)
    $docRelative = [IO.Path]::GetRelativePath($DocumentationRoot, $sourceFile.FullName).Replace('\', '/')
    if ($docRelative -in $generatedFiles) { continue }
