...
assert.deepEqual(read.Data, { header: { kind: 2, length: 6 } });
...
assert.equal(largeRead.Data.large.value, "18446744073709551615");
...
assert.deepEqual(metadataRead.Data.root, metadata);
const metadataRoundTrip = await serialize(metadataDefinition, metadataRead.Data.root, metadataOptions);
...
assert.equal(metadataUpdatedRead.Data.root.label, "£\0");
...
for (const source of [largeInput, new DataView(largeInput.buffer), new Blob([largeInput]),
  ...
  assert.deepEqual(parsed.Data, { header: { kind: 2, length: 6 } });
...
try {
  ...
  assert.deepEqual(a.Data.root, metadata);
