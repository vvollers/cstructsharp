...
every operation, including typed reads and POCO writes. Mapping uses reflection, so two conventions apply to your
own classes: mark the classes reached *through* the mapped type with `[DynamicallyAccessedMembers(...)]`, and
declare collection members as `List<T>` or `T[]` rather than a collection interface. `dynamic` access is JIT-only.
[Trimming and Native AOT](trimming-and-native-aot.md) has the details, the exact attribute, and the messages a
missed convention produces; `tests/CStructSharp.AotConsumer` runs those cases in CI.

Next, read [Write and serialize values](writing-and-serialization.md) to use POCOs and parsed values as output.
