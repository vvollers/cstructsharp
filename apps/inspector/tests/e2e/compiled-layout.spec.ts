...
test("compiled browser layouts retain decisions per read and recover after cancellation", async ({ page }) => {
  await page.goto("/");
  ...
  const results = await page.evaluate(async () => {
    ...
    const layout = await api.compile("struct root { uint8 tag; switch (tag) { case 1: { uint32 value; } case 2: { uint16 small; } default: { uint8 fallback; } } };");
    const values = [new Uint8Array([1,42,0,0,0]), new Uint8Array([2,19,0]), new Uint8Array([3,7])];
    try {
      ...
      let abortName = "";
      try { await layout.parse(values[0]!, {signal:cancelled.signal}); }
      catch (error) { abortName = (error as Error).name; }
      const recovered = await layout.parse(values[1]!);
      await layout.dispose();
      let disposed = false;
      try { await layout.parse(values[0]!); } catch { disposed = true; }
  ...
  expect(results.parsed.map(result => JSON.parse(result.Data!))).toEqual([
    {root:{tag:1,value:42}}, {root:{tag:2,small:19}}, {root:{tag:3,fallback:7}},
  ]);
